...
- **`配套图片`**：完型、阅读或新题型中的关联插图已进行统一命名处理，避免题号覆盖冲突，以卷内相对路径（`assets/`）在 JSON 中引用。

- **`index.json` 全量索引**：根目录的机读索引文件，汇总各卷的文件路径、章节题型、题量与分值（44 卷 / 1998–2025 / 共 2152 题），程序化调用建议先读索引再按需加载。

目录结构：全部年份卷目统一存放于 `data/` 目录（如 `data/2024-1/` 即 2024 年英语一）。
